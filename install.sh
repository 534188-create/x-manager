...
if [ "$MODE" = "interactive" ]; then
    ...
    echo -e "  ${YELLOW}[1]${NC} ⚡ ${BOLD}Быстрый старт${NC} (Автоматическая установка «под ключ» с лучшими пресетами)"
...
echo ""
echo -e "${CYAN}==> Шаг 1: Установка необходимых системных утилит...${NC}"
export DEBIAN_FRONTEND=noninteractive
apt-get update -qq || true
apt-get install -y -qq curl wget jq unzip iptables qrencode openssl python3 iproute2 >/dev/null 2>&1 || true

echo -e "${CYAN}==> Шаг 2: Настройка внутренних шлюзов ядра Xray (3X-UI)...${NC}"
...
try:
    ...
    if row:
        ...
        existing_tags = [ib.get("tag") for ib in inbounds]
        modified = False

        # in-mieru-socks (:10808)
        if "in-mieru-socks" not in existing_tags:
            inbounds.append({
                "tag": "in-mieru-socks",
                "port": 10808,
                "protocol": "socks",
        ...
        # in-snell-redirect (:12346)
        if "in-snell-redirect" not in existing_tags:
            inbounds.append({
        ...
        # in-wdtt-tproxy (:12345)
        if "in-wdtt-tproxy" not in existing_tags:
            inbounds.append({
                "tag": "in-wdtt-tproxy",
                "port": 12345,
                "protocol": "dokodemo-door",
        ...
        if modified:
            ...
            conn.commit()
            print("  ✓ Шлюзы Xray (TPROXY :12345, REDIRECT :12346, SOCKS5 :10808) успешно добавлены в шаблон 3X-UI")
        else:
            print("  ✓ Шлюзы Xray уже присутствуют в конфигурации 3X-UI")
    conn.close()
except Exception as e:
    print("  ✗ Ошибка интеграции с 3X-UI:", e)
EOF
    systemctl restart x-ui 2>/dev/null || true
    sleep 2
else
    echo -e "${YELLOW}  ! 3X-UI не обнаружен (/etc/x-ui/x-ui.db не найден). Пропускаем внедрение в базу.${NC}"
fi
...
EOF
    ...
    # Скрипт маршрутизации snell-routing.sh
...
if [ "$MODE" = "xray" ]; then
    ...
    iptables -t nat -A SNELL_OUT -d 89.19.223.185 -j RETURN 2>/dev/null || true
        ...
        cat << EOF > /etc/mieru/mita.json
{
  ...
  "egress": {
    ...
    "proxies": [
      { "name": "xray_socks", "protocol": "PROTOCOL_SOCKS5", "ip": "127.0.0.1", "port": 10808 }
    ]
  },
  "logging": { "level": "INFO" }
}
...
iptables -t mangle -A WDTT_TPROXY -p tcp -j TPROXY --on-port 12345 --tproxy-mark 1
iptables -t mangle -A WDTT_TPROXY -p udp -j TPROXY --on-port 12345 --tproxy-mark 1
...
if [ "$DEFAULT_ROUTING" = "xray" ]; then
    ...
    echo -e "  ✓ WDTT TPROXY маршрутизация активирована"
else
...
iptables -I INPUT 1 -i "$WAN_IF" -p tcp --dport 12345 -j DROP 2>/dev/null || true
iptables -I INPUT 1 -i "$WAN_IF" -p udp --dport 12345 -j DROP 2>/dev/null || true
iptables -I INPUT 1 -i "$WAN_IF" -p tcp --dport 12346 -j DROP 2>/dev/null || true
echo -e "  ✓ Внутренние порты ядра Xray (12345, 12346) защищены от внешнего доступа"
